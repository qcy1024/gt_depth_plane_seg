...
plane_seg_img = ti.field(dtype=ti.f32, shape=(10,12,2))
@ti.kernel
def set_depth_initval():
    for i, j in depth_gt_img:
        depth_gt_img[i, j] = 3.14
    return 
set_depth_initval()

point_cloud_img = ti.field(dtype=ti.f32, shape=(20,20,3))

img_depth_gt_field = depth_gt_img
img_objseg_gt_field = objseg_gt_img
# @ti.func
# def recal():
#     return 
...
planeinfo_type = ti.types.struct(plane_idx=int, error=float, a=float, b=float, d=float, x=int, y=int )
pixels = ti.field(dtype=ti.f32, shape=(17,25))
